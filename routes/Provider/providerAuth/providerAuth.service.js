...
module.exports = {
    ...
    GetServiceNameService: async (masterId, cat_id, callback) => {
        const getServiceNameQuery = process.env.GET_SERVICE_NAME_SERVICE
            .replace('<masterId>', masterId)
            .replace('<cat_id>', cat_id)
        console.log('getServiceNameQuery: ', getServiceNameQuery);
        pool.query(getServiceNameQuery, (err, result) => {
            if (err) {
                console.error("Error getting service name:", err);
                return callback(err);
            }
            return callback(null, result);
        });
                    ...
                    pool.query(categoryQuery, [categoryValues], async (err, result) => {
                        ...
                        if (email) {
                            const emailPayload = {
                                ...
                                data: { name, otp, masterName, catName },
                            };
